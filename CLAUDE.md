...
- Commits MUST be signed. Never use `--no-gpg-sign`
  or `commit.gpgsign=false`. Remote sessions (Claude
  Code on the web) satisfy this automatically: the
  environment sets `commit.gpgsign=true` with its own
  SSH key and GitHub shows the commits as Verified,
  so plain `git commit` is enough there and the rule
  below never applies.
- Local sessions only: when `git commit -S` fails
  (`agent refused operation`, `Couldn't get agent
  socket`, signing timeout), it is ALWAYS the same
  cause: the user is away from their computer, so
  Secretive can't authorize the key. Do NOT diagnose
  it, retry it, probe `/tmp` ssh-agent sockets,
  test-sign, or hunt for a working `SSH_AUTH_SOCK` —
  none of that ever helps and it burns tokens every
  time. The instant a signed commit fails: STOP. Leave
  the change staged (don't pile on more edits), tell
  the user signing needs them back at their machine,
  and move on / wait. Resume only when they say
  signing is back.
