...
func TestGenerationsDiffSubcommandGone(t *testing.T) {
	for _, c := range rootCmd.Commands() {
		if c.Name() != "generations" {
			continue
		}
		for _, sub := range c.Commands() {
			if sub.Name() == "diff" {
				t.Fatal("generations diff must not be registered")
			}
		}
		return
	}
...
func TestGenerationsRemoveSubcommandGone(t *testing.T) {
	for _, c := range rootCmd.Commands() {
		if c.Name() != "generations" {
			continue
		}
		for _, sub := range c.Commands() {
			if sub.Name() == "remove" {
				t.Fatal("generations remove must not be registered")
			}
		}
		return
	}
