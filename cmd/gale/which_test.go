package main

import (
	"os"
	"path/filepath"
	"slices"
	"testing"
)

func TestResolveWhich(t *testing.T) {
	t.Run("resolves binary to package", testResolveWhichResolves)
	t.Run("binary not found", testResolveWhichMissing)
	t.Run("broken symlink", testResolveWhichBrokenSymlink)
	t.Run("rejects store path missing bin segment", testResolveWhichMissingBin)
	t.Run("git hash version", testResolveWhichGitHash)
}

func whichFixture(t *testing.T) (galeDir, storeRoot string) {
	t.Helper()
	galeDir = t.TempDir()
	storeRoot = filepath.Join(galeDir, "pkg")
	return galeDir, storeRoot
}

func testResolveWhichResolves(t *testing.T) {
	galeDir, storeRoot := whichFixture(t)
	pkgDir := mkStorePkg(t, storeRoot, "jq", "1.8.1")
	binPath := filepath.Join(pkgDir, "bin", "jq")
	mkActiveGen(t, galeDir, 1, binPath)

	got, err := resolveWhich("jq", galeDir, storeRoot)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.name != "jq" {
		t.Errorf("name = %q, want %q", got.name, "jq")
	}
	if got.version != "1.8.1" {
		t.Errorf("version = %q, want %q", got.version, "1.8.1")
	}
	// EvalSymlinks to handle macOS /var → /private/var.
	wantPath, _ := filepath.EvalSymlinks(binPath)
	if got.path != wantPath {
		t.Errorf("resolved = %q, want %q",
			got.path, wantPath)
	}
}

func testResolveWhichMissing(t *testing.T) {
	galeDir, storeRoot := whichFixture(t)
	mkActiveGen(t, galeDir, 1)

	if _, err := resolveWhich("nonexistent", galeDir, storeRoot); err == nil {
		t.Fatal("expected error for missing binary")
	}
}

func testResolveWhichBrokenSymlink(t *testing.T) {
	galeDir, storeRoot := whichFixture(t)
	// mkActiveGen names the link filepath.Base(target),
	// which would be "path", not "broken".
	binDir := filepath.Join(galeDir, "gen", "1", "bin")
	if err := os.MkdirAll(binDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("/nonexistent/path",
		filepath.Join(binDir, "broken")); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(
		filepath.Join("gen", "1"),
		filepath.Join(galeDir, "current"),
	); err != nil {
		t.Fatal(err)
	}

	if _, err := resolveWhich("broken", galeDir, storeRoot); err == nil {
		t.Fatal("expected error for broken symlink")
	}
}

func testResolveWhichMissingBin(t *testing.T) {
	galeDir, storeRoot := whichFixture(t)
	// Malformed store entry without bin/: pkg/jq/1.8.1/jq.
	badPath := filepath.Join(storeRoot, "jq", "1.8.1", "jq")
	if err := os.MkdirAll(filepath.Dir(badPath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(badPath, []byte("fake"), 0o755); err != nil {
		t.Fatal(err)
	}
	mkActiveGen(t, galeDir, 1, badPath)

	if _, err := resolveWhich("jq", galeDir, storeRoot); err == nil {
		t.Fatal("expected error for path missing bin/ segment")
	}
}

func testResolveWhichGitHash(t *testing.T) {
	galeDir, storeRoot := whichFixture(t)
	pkgDir := mkStorePkg(t, storeRoot, "gale", "d871cf2")
	mkActiveGen(t, galeDir, 1, filepath.Join(pkgDir, "bin", "gale"))

	got, err := resolveWhich("gale", galeDir, storeRoot)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.name != "gale" {
		t.Errorf("name = %q, want %q", got.name, "gale")
	}
	if got.version != "d871cf2" {
		t.Errorf("version = %q, want %q",
			got.version, "d871cf2")
	}
}

// TestOtherProvidersReportsShadowedPackage covers a pre-upgrade
// generation: two packages both ship foo, only beta's copy is
// on PATH, and `which` reports the loser. A successful rebuild
// cannot produce this state — leftover [bin] no longer settles
// a collision — so the generation is assembled by hand.
func TestOtherProvidersReportsShadowedPackage(t *testing.T) {
	galeDir, storeRoot := setupGCHome(t)

	alphaDir := mkStorePkg(t, storeRoot, "alpha", "1.0")
	betaDir := mkStorePkg(t, storeRoot, "beta", "1.0")
	addStoreBin(t, alphaDir, "foo")
	addStoreBin(t, betaDir, "foo")
	gammaDir := mkStorePkg(t, storeRoot, "gamma", "1.0")

	mkActiveGen(
		t, galeDir, 1,
		filepath.Join(alphaDir, "bin", "alpha"),
		filepath.Join(betaDir, "bin", "beta"),
		filepath.Join(betaDir, "bin", "foo"),
		filepath.Join(gammaDir, "bin", "gamma"),
	)

	winner, err := resolveWhich("foo", galeDir, storeRoot)
	if err != nil {
		t.Fatalf("resolveWhich: %v", err)
	}
	if winner.name != "beta" {
		t.Fatalf("winner = %q, want beta", winner.name)
	}

	got := otherProviders("foo", winner.name, galeDir, storeRoot)
	want := []string{"alpha"}
	if !slices.Equal(got, want) {
		t.Errorf("otherProviders = %v, want %v — gamma ships no foo, "+
			"beta is the winner", got, want)
	}
}
