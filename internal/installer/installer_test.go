...
func TestInstallBinaryNonGHCRDefaultTrustFails(t *testing.T) {
	...
	// Source fallback is gone, so the error is the result.
	_, err = inst.Install(context.Background(), r)
	if err == nil {
		t.Fatal("non-GHCR default-trust binary must refuse")
	}
	if !strings.Contains(err.Error(), "trust") &&
		!strings.Contains(err.Error(), "sigstore") &&
		!strings.Contains(err.Error(), "fetch") {
		t.Errorf("refusal must name trust, sigstore, or fetch: %v", err)
	}
}
...
func TestInstallBinarySigstoreTrustUnavailableVerifierFails(t *testing.T) {
	...
	// The rejection reason must name attestation.
	if err == nil || !strings.Contains(err.Error(), "attestation") {
		t.Errorf("binary rejection does not mention attestation; err=%v", err)
	}
}
